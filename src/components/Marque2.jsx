        ...
        <section className='flex flex-col relative h-[12rem] pt-6 overflow-hidden'>
            <div className='marquee2 font-clash tracking-widest text-main_primary/50'>
                {Array.from({ length: 3 }).map((_, index) => {
                    return <h4 className="text font-black">SOLVE-CREATE-THRIVE</h4>
                })}
            </div>
            ...
            {Array.from({ length: 5 }).map((_, index) => {
                return <>
                    <h1 className="text font-black textStroke text-transparent">{shortText}{String.fromCharCode(160).repeat(longText.length - shortText.length)}</h1>
